	...
	public void acheterProduit(String nomAcheteur) {
		...
		if(!acheteurReconnu) {
			...
			if (donneesEtalProduit!=null){
				...
				int quantiteVoulu = Clavier.entrerEntier("Bonjour " + nomAcheteur + "\nCombien de " + produit + " voulez-vous acheter ?");
				int quantiteAcheter = controlAcheterProduit.acheterProduitVendeur(quantiteVoulu, nomVendeur);
				if (quantiteAcheter==0) {
					System.out.println(nomAcheteur + " veut acheter " + quantiteVoulu  + " "+ produit + ", malheureusement il n'y en a plus !");
					...
					System.out.println(nomAcheteur + " veut acheter " + quantiteVoulu  + " " + produit + ", malheureusement " 
				          + nomVendeur + " n'en a plus que " + quantiteAcheter + ". " + nomAcheteur + " achète tout le stock de " + nomVendeur + ".");
				}
